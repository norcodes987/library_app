...
export const ChangeQuantityOfBook: React.FC<{ book: BookModel, deleteBook: any }> = (props, key) => {
    ...
    const [quantity, setQuantiity] = useState<number>(0);
    ...
    useEffect(() => {
        const fetchBookInState = () => {
            props.book.copies ? setQuantiity(props.book.copies) : setQuantiity(0);
    ...
    async function increaseQuantity() {
        ...
        const quantityUpdateResponse = await fetch(url, reqOptions('PUT'));
        if (!quantityUpdateResponse.ok) {
            throw new Error('Something went wrong!');
        }
        setQuantiity(quantity+1);
    ...
    async function deleteBook () {
        ...
        const updateResponse = await fetch(url, reqOptions('DELETE'));
