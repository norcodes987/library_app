...
export const AdminMessage: React.FC<{message: MessageModel, submitResponseToqQuestion: any}> = (props, key) => {
    const [displayWarning, setDisplayWarning] = useState<boolean>(false);
    const [response, setResponse] = useState<string>('');

    function submitBtn() {
        if (props.message.id !== null && response !== '') {
            props.submitResponseToqQuestion(props.message.id, response);
